...
class VideoTapeRepository {
    ...
    public static function video_views($video_tape_id,$user_id,$type){
        // $video = VideoViews::where('video_id',$video_tape_id)->get();
        $video = VideoViews::where('video_id',$video_tape_id)->first();
        //  echo "<pre>";
        // print_r($video);
        // echo "<pre>";
        // print_r($video_tape_id);
        // echo "<pre>";
        // print_r($user_id);

        $clintip=$_SERVER['REMOTE_ADDR'];
        if($video->video_id != $video_tape_id && $video->ip != $clintip ){

            $details=array(
             'video_id' =>$video_tape_id,
             'ip'       =>$clintip,
             'user_id'  =>$user_id
            );

            VideoViews::create($details);
        }
        // exit;
    ...
    public static function watch_count($video_tape_id,$user_id,$type) {
        ...
        if($video = VideoTape::where('id',$video_tape_id)
            ...
            $video_views = VideoViews::where('video_id',$video_tape_id)->first();
            $clintip=$_SERVER['REMOTE_ADDR'];

            if($video_views->video_id != $video_tape_id && $video_views->ip!= $clintip ){
                $video->watch_count += 1;
            }
            $video->save();
