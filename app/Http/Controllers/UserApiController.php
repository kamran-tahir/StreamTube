...
class UserApiController extends Controller {
        ...
        if ($validator->fails()) {
            ...
            if($user) {
                ...
                if ($user->login_by == 'manual') {
                    ...
                    $body = View::make($page,array('email_data' => $email_data,'site_url' => url('/')))->render();
                    $email_send_response = Helper::sendphpMailerEmail($user->email,$user->name,$subject,$body);
    ...
    /**
     * @method featured_list()
     *
     * @usage_place : WEB
     *
     * @uses To display the featured videos
     *
     * @param object $request - User Details
     *
     * @return Response of videos list
     */
    public function featured_list($request) {
     ...
     */
    public function channel_videos($channel_id, $skip , $request = null, $is_member = null) {
        ...
        if($is_member){
            $videos_query->where('video_tapes.member_only', $is_member);
        }
    ...
    public function video_detail(Request $request) {
        ...
        if ($video) {
            ...
            $videoComments = VideoTape::find($video->video_tape_id)->load('comments','comments.replies', 'comments.user', 'comments.replies.user');
            ...
            $response_array = [
                ...
                'video'=>$video, 
                'comments'=>$comments, 
                'comments_count'=>$videoComments->comments_count, 
    ...
    */
    ...
    public function channel_member_users($channel_id,$skip=0){
        $channel = Channel::find($channel_id);
        $users = $channel->channel_members();
        
        if ($skip >= 0) {

            //Setting::get('admin_take_count', 12)
            $users = $users->skip($skip)->take(Setting::get('admin_take_count', 12))->get();
    ...
    public function channel_posts($channel_id,$category_id=null,$user_id=null,$without_pagination=false,$search=null,$type=null) 
    {
        $posts = ChannelPost::where('channel_id',$channel_id);
        if($user_id){
        ...
        if($type){
            if($search){
                $posts = $posts->where($type,'like','%'.$search.'%');
            }    
        }else{
            if($search){
    ...
    public function forum_categories_list(Request $request) {

        $model = ForumCategory::orderBy('created_at', 'desc')->get();

        return response()->json($model);
    
    }

    public function channel_photos($channel_id,$skip , $request = null, $is_member = null) 
    {
        $photos = Photo::where('channel_id',$channel_id);

        if($is_member){
            $photos->where('member_only', $is_member);
        }

        if ($skip >= 0) {

            //Setting::get('admin_take_count', 12)
            $photos = $photos->skip($skip)->take(Setting::get('admin_take_count', 12))->get();

        } else {

            $photos = $photos->paginate(16);
        }
