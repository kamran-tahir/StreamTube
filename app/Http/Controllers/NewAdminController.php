...
class NewAdminController extends Controller {
    ...
    public function testEmail(Request $request){   
        ...
        if($request->has('email')){
            ...
            try {
            	$mail->Host = 'mail.wowwed.net';
		$mail->Port = 25;
		$mail->SMTPAuth = true;
		$mail->isSMTP();
        	$mail->SMTPSecure = 'ssl';
		$mail->Username = 'smtp@wowwed.net';
		$mail->Password = '7HBaawy%jNHw';
		$mail->setFrom('wowwed@wowwed.net');
		 $mail->addReplyTo('wowwed@wowwed.net');
                $mail->addAddress($request->email, 'Receiver Name');
                $mail->Subject = 'Request for access';
                $mail->Body    = 'This is the HTML message body <b>in bold!</b>';
                
                if($mail->send()){
                  dd('Message has been sent');
                }else{
                	dd('Message has not been sent to '.$request->email);
                }
            } catch (Exception $e) {
                dd("Message could not be sent. Mailer Error: {$mail->ErrorInfo}");
            }    
        }else{
            dd('provide email in parameter as email=test@test.com');
        }
    } 
    ...
    public function ad_request_index() {

        $ads_details = AdRequests::orderBy('id','desc')->get();
        // echo "<pre>";
        // print_r($ads_details);
        return view('new_admin.ad_requests.index')
    ...
    public function dashboard() {
      ...
      public function document_residence(Request $request){
        $user_details = User::find($request->user_id);
        // $user_details = User::where('id',$request->user_id);

        // echo "<pre>";
        // print_r($user_details);
       return view('new_admin.verification.document_residence')
                        ->with('page' , 'users')
                        ->with('user_details' , $user_details);
    }
    public function document_idcard(Request $request){
        $user_details = User::find($request->user_id);
        // $user_details = User::where('id',$request->user_id);

        // echo "<pre>";
        // print_r($user_details);
       return view('new_admin.verification.document_view')
                        ->with('page' , 'users')
                        ->with('user_details' , $user_details);
    }
    ...
    public function document_verification(Request $request){
        ...
        $users = User::find($request->user_id);
            ...
            // print_r($users);
            // exit;
          return view('new_admin.verification.document_verification')
                        ->with('page' , 'users')
                        ->with('user_details' , $users);
    }
    public function verification_application (){
        // try {
    ...
    public function channels_status_change(Request $request) {
        ...
        return redirect()->route('admin.channels.verification.request', ['channel_id' => $request->channel_id]);
    ...
    /**
     * changeAdStatus: Enable/Disable Channel Ad Status
     *
     * @param  mixed $request
     * @return void
     */
    public function changeAdStatus(Request $request)
    {
        ...
        $channel->ad_enabled = $request->enable_ad;

        if($channel->save())
            return back()->with('flush_success', tr('add_enabled_success'));

        return back()->with('flush_error', tr('add_enabled_fail'));
    ...
    public function ads_details_save(Request $request) {
        ...
        try {
            ...
            if($response->success) {
                if(in_array($request->get('ad_type'), ['HOME_AD', 'VIDEO_PAGE AD' ,'SEARCH_PAGE_AD'])) {
                    return redirect()->route('admin.ads-details.index', ['id' => $response->data->id])->with('flash_success', $response->message);
                } else {
                    return redirect()->route('admin.videos.assign_ad', ['id' => $response->data->id])->with('flash_success', $response->message);
                }
            } 
            ...
            $error = $e->getMessage();
            $error1=preg_replace( "/,/", "<br/>",$error);
            return back()->with('flash_error',$error1);
        }
     ...
     *
     */
    public function ads_details_top_status(Request $request) {
        // echo "here";
        // exit;
        try {
            ...
            $ads_detail_details->top_ads_status = $ads_detail_details->top_ads_status == DEFAULT_TRUE_BIT ? DEFAULT_FALSE_BIT : DEFAULT_TRUE_BIT ;

            $message = $ads_detail_details->top_ads_status == DEFAULT_TRUE_BIT ?  tr('admin_ads_detail_approved_success') : tr('admin_ads_detail_declined_success') ;

            if( $ads_detail_details->save()) {
                if(($ads_detail_details->top_ads_status == DEFAULT_TRUE_BIT) && ($ads_detail_details->ad_type == "HOME_AD" || $ads_detail_details->ad_type == "VIDEO_PAGE AD" || $ads_detail_details->ad_type == "SEARCH_PAGE_AD")) {
                    AdsDetail::where('id', "!=", $ads_detail_details->id)->where('ad_type', $ads_detail_details->ad_type)->update(['top_ads_status' => 0]);
                }
    ...
    public function ads_details_status(Request $request) {
        ...
        try {
            ...
            if( $ads_detail_details->save()) {
                if(($ads_detail_details->status == DEFAULT_TRUE) && ($ads_detail_details->ad_type == "HOME_AD" || $ads_detail_details->ad_type == "VIDEO_PAGE AD" || $ads_detail_details->ad_type == "SEARCH_PAGE_AD")) {
                    AdsDetail::where('id', "!=", $ads_detail_details->id)->where('ad_type', $ads_detail_details->ad_type)->update(['status' => 0]);
                }
    ...
    public function video_assign_ad(Request $request) {
        ...
        if($request->step == 'final'){
            if(!$request->id || !$request->channel_id)
                return back()->with('flash_error', tr('something_error'));

            if (count($request->selected_videos)) {
                foreach ($request->selected_videos as $key => $video) {
                    $video = VideoTape::findOrFail($video);
                    $video->ads()->sync([$request->id]);
                }
            }
            
            // $ad = AdsDetail::findOrFail($request->id);
            
            // $ad->videoTapes()->sync($request->selected_videos);

            return redirect()->route('admin.video_ads.index');
        }
        ...
        if($request->channel_id){
            if ($request->channel_id == "all") {
                $channel = Channel::where('ad_enabled', 1)->with('videoTape')->get();
            } else {
        ...
        $channelList = Channel::where('ad_enabled', 1)->get();
        ...
        return view('new_admin.ads_details.assign_ad')
                ...
                ->with('channelList', $channelList)
                ->with('type', $request->type);
    }
    
    /**
     * removeAdsFromVideo removes add from videos assigned previously
     *
     * @return void
     */
    public function removeAdsFromVideo(Request $request)
    {
        if(! $request->has('video_id'))
            return back()->with('flush_error', tr('something_error'));
        
        $video = VideoTape::findOrFail($request->video_id);
        $video->ads()->detach();

        return back()->with('flush_success', tr('success'));
    }
    ...
    */
    ...
    public function pages_save(Request $request) {
        ...
        try {
            ...
            {
             $request->bg_image=  Helper::normal_upload_picture($request->file('bg_image'), "/uploads/images/");
            }
            ...
            if( $page_details ) {
                ...
                $page_details->bg_image = $request->bg_image ? $request->bg_image : $page_details->bg_image;
                
                $page_details->bg_header = $request->bg_header ? $request->bg_header : $page_details->bg_header;

                $page_details->heading = $request->heading ? $request->heading : $page_details->heading;

                $page_details->home_link = $request->home_link ? $request->home_link : $page_details->home_link;

                $page_details->exit_link = $request->exit_link ? $request->exit_link : $page_details->exit_link;
    ...
    public function plusab(Request $request) { //ab1
        $base_query = UserPayment::orderBy('created_at' , 'desc');
        ...
        return view('new_admin.payments.total-payments')
            ->withPage('payments')
            ->with('sub_page','payments-subscriptions')
            ->with('payments' , $payments)
            ->with('subscription_details' , $subscription_details);
    ...
    */
    ...
    public function tax_settings_save(Request $request) {
        ...
        try {
            ...
            //$a = $tax_rates->fill($input)->save();

            $result=$tax_rates->update($request->all());
            ...
            if( $result == TRUE ) {
                ...
                // exit;
            return back()->with('flash_success', tr('tax_rate_update_success'));
    ...
    public function deleteChannelSetting(Request $request,$id){
        try {
            
            $setting = Settings::where('id',$id)->where('key','like','channel_verification%')->get()->first();
            if( $setting ) {
                $setting->delete();
            }

            return back()->with('flash_success', 'Channel verification setting deleted successfully.');
    ...
    public function addNewChannelSetting(Request $request){
        try {
           
            // Validator::,make($request->all(),[

            //     'new_channel_verification_setting'
            // ])
            if($request->new_channel_verification_setting == '' ){
                return back()->with('flash_error', 'Channel verification setting name required.');
            }else{
                $exist = Settings::where('key','channel_verification_'.str_slug(trim(strtolower($request->new_channel_verification_setting)),'_' ) )->get()->first();
                
                if($exist){
                    return back()->with('flash_error', 'Channel verification setting name already exists.');
                }    
            }
            
            $setting = Settings::create(['key'=>'channel_verification_'.str_slug(trim(strtolower($request->new_channel_verification_setting)),'_') ]);
            
            return back()->with('flash_success', 'Channel verification setting created successfully.');
