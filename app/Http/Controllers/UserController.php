...
class UserController extends Controller
{
     ...
     */
    public function ad_submit_save(Request $request)
    {
        ...
        $validator = Validator::make(
            ...
            array(
                ...
                'address'    => 'required',
                'ad_type'    => 'required',
                'ad_skip_time'    => 'required',
                'ad_duration' => 'required',
                'url'        => 'required',
                'file'       => 'mimes:jpeg,bmp,png,mp4,flv,avi'
        ...
        if ($request->hasFile('file') != "") {
            $request->file =  Helper::normal_upload_picture($request->file('file'), "/uploads/images/");
        }

        $model = new AdRequests();
        ...
        $model->ad_type    = $request->ad_type;
        $model->ad_skip_time    = $request->ad_skip_time;
        $model->ad_duration    = $request->ad_duration;
        $model->url        = $request->url;
        $model->file       = $request->file;
        if ($model->save()) {
            //  $user = User::find($request->id)->update($details);
            return redirect(route('user.dashboard'))->with('flash_success', tr('ad_success'));
        }
    }
    public function ad_submit(Request $request)
    {
        ...
        // $settings= AdRequests::orderBy('id')->get();
        // echo "<pre>";
        // print_r($settings);

        return view('user.ad_submit');
    ...
    public function photos_gallery(Request $request, $channel_id = null)
    {
        $photo_id = $request->query('photo_id') ?? null;

        if (Auth::check()) {
            if ($photo_id) {
                $photo = Photo::find($photo_id);
                if ($photo->members_only) {
                    if (!userChannelMemberStaus(Auth::user(), $photo->channel_id) and !Auth::user()->getChannel->contains($photo->channel_id)) {
                        session()->flash('flash_error', 'Sorry, You are not eligible to see the photo. Become Member of the channel');
                        return redirect()->route('user.channel', $photo->channel_id);
                    }
                }
            }
            $is_member_section = $request->query('is_member_section') ?? null;

            if ($is_member_section) {
                $members_channels = auth()->user()->member_of->pluck('id')->toArray();
                $user_channels = auth()->user()->getChannel->pluck('id')->toArray();

                $members_only_photos = Photo::where('members_only', 1);

                if ($channel_id) {
                    if (in_array($channel_id, $members_channels) ||  in_array($channel_id, $user_channels)) {
                        $members_only_photos = $members_only_photos->where('channel_id', $channel_id)->get();
                    } else {
                        $members_only_photos = null;
                    }
                } else {
                    $members_only_photos = null;
                }
                $user_photo = new Collection(); //Create empty collection which we know has the merge() method
                $user_photo = $user_photo->merge($members_only_photos);
            } else {
                $user_photo_public = Photo::where('members_only', 0);
                if ($channel_id) {
                    $user_photo_public = $user_photo_public->where('channel_id', $channel_id);
                }
                $user_photo_public = $user_photo_public->where('user_id', '!=', auth()->id())->get();

                $my_photos = Photo::where('user_id', auth()->id());
                if ($channel_id) {
                    $my_photos = $my_photos->where('channel_id', $channel_id);
                }
                $my_photos = $my_photos->get();

                $members_channels = auth()->user()->member_of->pluck('id')->toArray();
                $members_only_photos = Photo::where('members_only', 1)->where('user_id', '!=', auth()->id());
                if ($channel_id) {
                    if (in_array($channel_id, $members_channels)) {
                        $members_only_photos = $members_only_photos->where('channel_id', $channel_id)->get();
                    } else {
                        $members_only_photos = null;
                    }
                } else {
                    $members_only_photos = $members_only_photos->whereIn('channel_id', $members_channels)->get();
                }

                $user_photo = new Collection(); //Create empty collection which we know has the merge() method
                $user_photo = $user_photo->merge($user_photo_public);
                $user_photo = $user_photo->merge($my_photos);
                $user_photo = $user_photo->merge($members_only_photos);
                $user_photo = $user_photo->sortByDesc('id')->unique();
            }
        } else {
            if ($photo_id) {
                $photo = Photo::find($photo_id);
            ...
            $user_photo = Photo::where('members_only', 0);
            if ($channel_id) {
                $user_photo = $user_photo->where('channel_id', $channel_id);
            }
            $user_photo = $user_photo->latest()->get();
        }
    ...
    public function index(Request $request)
    {
        ...
        if ($database && $username && Setting::get('installation_process') == 2) {
            ...
            $side_ad = AdsDetail::select('id', 'file', 'name', 'ad_url as link')
                ->where('status', 1)
                ->where('ad_type', "HOME_AD")
                ->orderBy('id', "DESC")
                ->limit(1)
                ->get();
            /////////////////////////////////////////////////
            $top_side_ad = AdsDetail::select('id', 'file', 'name', 'ad_url as link')
                ->where('top_ads_status', 1)
                ->where('ad_type', "HOME_AD")
                ->orderBy('id', "DESC")
                ->limit(1)
                ->get();
            // dd($recent_videos->items);
            ...
            return view('user.index')
                ...
                ->with('side_ad', $side_ad)
                ->with('top_side_ad', $top_side_ad)
                ->with('users', $user);
        } else {
    ...
    public function history(Request $request)
    {
        ...
        $base_query = VideoTape::where('video_tapes.is_approved', '=', 1)
            ...
            ->leftJoin('user_search_history', 'user_search_history.video_id', '=', 'video_tapes.id')
            ...
            ->where('user_search_history.user_id', auth()->id());
        if ($search) {
            $base_query = $base_query->where(function ($q) use ($search) {
                $q->where('video_tapes.title', 'like', '%' . $search . '%')->orWhere('video_tapes.description', 'like', '%' . $search . '%');
            });
        }
        $base_query = $base_query->select('video_tapes.*', 'video_tapes.id as video_tape_id', 'user_search_history.id as search_history_id', DB::raw('Date(user_search_history.updated_at) as search_history_date'))
            ->orderBy('user_search_history.updated_at', 'desc');


        // $base_query = UserSearchHistory::where('user_search_history.user_id' , auth()->id())
        //                     ->leftJoin('video_tapes' ,'user_search_history.video_id' , '=' , 'video_tapes.id')
        //                     ->leftJoin('categories' , 'categories.id' , '=' , 'video_tapes.category_id') 
        //                     ->leftJoin('channels' ,'video_tapes.channel_id' , '=' , 'channels.id')
        ...
        //                     ->where('channels.status', 1)
        //                     ->where('channels.is_approved', 1)
        //                     ->select('user_search_history.id as history_id',
        //                             'video_tapes.id as video_tape_id' ,
        //                             'video_tapes.title',
        //                             'video_tapes.description' , 
        //                             'video_tapes.duration',
        //                             'video_tapes.member_only',
        //                             'default_image',
        //                             'video_tapes.banner_image as video_image',
        ...
        //                     ->where('video_tapes.age_limit','<=', checkAge($request))
        //                     ->where('categories.status', CATEGORY_APPROVE_STATUS)
        //                     ->orderby('user_search_history.created_at' , 'desc');


        $flag_videos = flag_videos(auth()->id());
        ...
        if (count($videos) > 0) {
            ...
            foreach ($videos->items() as $key => $value) {
                $item = displayVideoDetails($value, auth()->id());
                $item['url'] = route('user.single', $item['video_tape_id']);
                $items[] = (object) $item;
            }
        ...
        // ]);
        $items = collect($items);
        $items = $items->groupBy('search_history_date');
        $histories = (object) ['items' => $items, 'pagination' => $pagination];
    ...
    public function channel_view($id, Request $request)
    {
        ...
        if ($channel) {
            ...
            $photos = $this->UserAPI->channel_photos($id, 0);
    ...
    public function video_view(Request $request)
    {
        ...
        if ($data->success) {
            ...
            if ($response->video->member_only) {
                if (Auth::check()) {

                    if (!userChannelMemberStaus(Auth::user(), $response->video->channel_id)) {
                        return redirect()->route('user.channel', $response->video->channel_id)->with('flash_error', 'Sorry, You are not eligible to see the video. Become Member of the channel');
                    }
                } else {
            ...
            if ($request->id != $response->video->channel_created_by) {
                ...
                if (Auth::user()->search_history_status) {
                    $searchRecord = UserSearchHistory::whereDate('updated_at', '=', Carbon::today()->format('y-m-d'))->where('user_id', $user_id)->where('video_id', $request->video_tape_id)->get()->first();
                    if ($searchRecord) {
                        $searchRecord->touch();
                        $searchRecord->save();
                    } else {
                        UserSearchHistory::create(['user_id' => $user_id, 'video_id' => $request->video_tape_id]);
                    }
                }
            }
            $FFmpeg = new \FFmpeg;
            ...
            return view('user.single-video.sample-video')
                ...
                ->with('comments_count', $response->comments_count)
    ...
    public function add_comment(Request $request)
    {
        ...
        $returnHTML = view('user.comments')->withComments($video->comments)->withVideoTapeId($video->id)->withCommentsCount($video->comments_count)->render();
        return response()->json(array('success' => true, 'html' => $returnHTML, 'comments_count' => $video->comments_count));
    }
    /**
     * Function Name : add_reply()
     * 
     * @uses To Add comment based on single video
     *
     * @created Vithya R
    ...
    public function add_reply(Request $request)
    {
        ...
        $currentComment = Comment::find($request->comment_id);
        $comment = new Comment;

        $comment->user()->associate($request->user());
        $comment->comment = $request->comment;
        $comment->parent_id = ($currentComment->parent_id) ? $currentComment->parent_id : $request->comment_id;

        $video = VideoTape::find($request->video_tape_id);
        $video->comments()->save($comment);

        $parentComment = Comment::with('replies')->find($comment->parent_id);

        $returnHTML = view('user.comments')->withComments($parentComment->replies)->withVideoTapeId($request->video_tape_id)->render();
        return response()->json(array('success' => true, 'html' => $returnHTML, 'parent_id' => $parentComment->id, 'comments_count' => $video->comments_count, 'replies_count' => $parentComment->replies_count));
    }
    ...
    public function photo_live(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            //DB::enableQueryLog();
            // $channel = Channel::where('user_id', Auth::user()->id)->where('id', $id)->first();
            $channels = Channel::where('user_id', Auth::user()->id)->where('is_approved', 1)->where('status', 1)->get();
    ...
    public function members(Request $request)
    {
        ...
        $id = $request->id;
        $videos = $this->UserAPI->channel_videos($id, 0, $request, $is_member = 1)->getData();
        $user_photo = Photo::where('user_id', Auth::user()->id)->where('channel_id', $id)->where('members_only', 1)->get();
        $forum_updates = ForumUpdate::where('channel_id', $id)->orderby('id', 'desc')->get();

        $users = $this->UserAPI->channel_member_users($id)->getData();
        $channel = '';
        ...
        $categories_list = $this->UserAPI->categories_list($request)->getData();
        $forum_categories = ForumCategory::where('channel_id', $channel->id)->orderBy('created_at', 'desc')->get();
        $tags = $this->UserAPI->tags_list($request)->getData()->data;
        ...
        return view('user.members.live')->with('model', $model)->with('page', 'videos')
            ...
            ->with('channel', $channel)
            ->with('users', $users)
            ->with('forum_categories', $forum_categories)
            ->with('photo', $user_photo)
    ...
    public function save_photos(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (Auth::check()) {
                ...
                $photo->members_only = $request->members_only;
                $photo->save();
                // print_r($photo);
                // exit;
            }
    ...
    public function channel_subscribers(Request $request)
    {
        ...
        $subscribers = ChannelSubscription::whereIn('channel_subscriptions.channel_id', $list)
            ->select(
                'channel_subscriptions.channel_id as channel_id',
                'channels.name as channel_name',
                'users.id as user_id',
    ...
    public function partialVideos(Request $request)
    {
        ...
        $videos = $this->UserAPI->channel_videos($request->channel_id, $request->skip, $request, $request->member_only)->getData();
    ...
    /**
     * Function Name : partialPhotos()
     *
     * @uses To get phptos details of channels photos using skip & take
     *
     * @created Vithya R
     *
     * @updated 
     ...
     * @return json response details
     */
    public function partialPhotos(Request $request)
    {
        ...
        $photos = $this->UserAPI->channel_photos($request->channel_id, $request->skip, $request, $request->member_only);

        $channel = Channel::find($request->channel_id);

        $view = View::make('user.photos.partial_photos')
            ->with('photos', $photos)
            ->with('channel', $channel)
            ->render();

        return response()->json(['view' => $view, 'length' => count($photos)]);
    }
    ...
    public function search_history_status(Request $request)
    {
        ...
        if (!$user_details) {
            ...
            return back()->with('flash_error', tr('user_not_found'));
        } else if ($user_details->id != Auth::id()) {
            return back()->with('flash_error', tr('user_not_found'));
        }

        $user_details->search_history_status  = $request->status;
        $user_details->save();
        if ($user_details->search_history_status) {
            $msg = 'Your Watch History Turned ON';
        } else {
            $msg = 'Your Watch History Turned OFF';
        }

        return redirect()->route('user.history')->with('flash_success', $msg);
    }
    ...
    public function clear_search_history(Request $request)
    {
        ...
        if ($request->record_id) {
            $searchRecord = UserSearchHistory::where('user_id', $user_details->id)->where('id', $request->record_id)->delete();
            $msg = "Selected Entry From Search History Has been cleared.";
        } else {
            $searchRecord = UserSearchHistory::where('user_id', $user_details->id)->delete();
            $msg = "All Search History been cleared.";
        }
