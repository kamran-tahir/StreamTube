...
<body>
    <header>
        ...
        <div class="navbar">
            ...
            <div class="brand">Channel Forum</div>
        </div>
        <!--SearchBox Section-->
        <!-- <div class="search-box">
            <form>
                <div>
                    ...
                    <a id="clear-search" href="{{route('user.channel.member-section.community', $channel)}}" style="display:none"></a>
                </div>
            </form>
        </div> -->
    </header>
    <div class="container">
        ...
        @endphp
        <div class="subforum" style="cursor:pointer;" onclick='clickCategory("category_{{$category_id}}")'>
            <div class="subforum-title">
                <h1> <a class="category-title" id="category_{{$category_id}}" href="{{route('user.channel.member-section.community.posts', ['channel'=> $channel,'category'=>$category_id]) }}"> {{$category->name}} </a></h1>
                <div class="category_posts_stats">
                    <span style="margin-right: 5%;">{{count($category_posts)}} Posts </span>
                    <span style="display: inline-block;">
                        <b><a href="">Last post</a></b> by <a href="">{{$category_posts->last()->user->name}}</a> 
                    <br>on <small> {{$category_posts->last()->created_at->format('d F Y')}}</small>
                    </span>
                </div>
