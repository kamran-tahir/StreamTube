        ...
        <div class="col-xs-12">
            ...
            <div class="box box-primary">
             ...
             <td><a href="{{route('admin.pages.view', ['page_id' => $page_details->id] )}}">{{$page_details->title}}</a></td>
                     ...
                     <span class="label label-success">{{ tr('activate') }}</span>

                                        @else

                     <span class="label label-warning">{{ tr('deactivate') }}</span>
...
<li><a onclick="return confirm(&quot;{{tr('page_delete_confirmation' , $page_details->title)}}&quot;);" href="{{ route('admin.pages.delete',['page_id' => $page_details->id] ) }}"><b>{{tr('delete')}}</b></a></li>
 ...
 @if($page_details->status == YES )
  <li role="presentation"><a role="menuitem" onclick="return confirm(&quot;{{ $page_details->name }} - {{ tr('admin_page_decline_confirmation') }}&quot;);" tabindex="-1" href="{{ route('admin.pages.status' , ['page_id' => $page_details->id]) }}"> {{ tr('deactivate') }}</a></li>
   @else 
