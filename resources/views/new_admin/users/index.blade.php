...
@section('content')
	...
	<div class="row">
        ...
        <div class="col-xs-12">
			...
			<a href="{{ route('admin.users.verify' ,['user_id' => $user_details->id] ) }}" class="btn btn-xs btn-success">{{ tr('verify') }}</a>
     		...
     		<span>{{ tr('verified') }}</span>
								      	...
								      	<td>								      		
			@if($user_details->status == USER_APPROVED)			<span class="label label-success">{{ tr('approved') }}</span>
			@else
	        ...
	        <ul class="admin-action btn btn-default">
	          ...
	          <li class="dropdown">									               
				...
				<li role="presentation"><a role="menuitem" tabindex="-1" onclick="return confirm(&quot;{{tr('admin_user_delete_with_expiry_days_confirmation') }}&quot;);" href="javascript:;" > {{ tr('delete') }} </a></li>	
	               ...
	               <li role="presentation">						
	               	<a role="menuitem" tabindex="-1" onclick="return confirm(&quot;{{ tr('admin_premium_user_delete_confirmation', $user_details->name ) }}&quot;);" href="javascript:;">{{ tr('delete') }}
					           	 	...
					           	 	@endif
              		@else
            	<li role="presentation"><a role="menuitem" tabindex="-1" href="{{ route('admin.users.edit' , ['user_id' => $user_details->id] ) }}">{{ tr('edit') }}</a></li>			                	@if(get_expiry_days($user_details->id) > 0)
				...
				@else 					                  			<li role="presentation">				      	<a role="menuitem" tabindex="-1" onclick="return confirm(&quot;{{ tr('admin_premium_user_delete_confirmation', $user_details->name ) }}&quot;);" href="{{ route('admin.users.delete', ['user_id' => $user_details->id] ) }}">{{ tr('delete') }}
						</a></li>
                  	 	@endif
		                  	@endif
        ...
        <a role="menuitem" tabindex="-1" href="{{ route('admin.users.document', ['user_id' => $user_details->id] ) }}" onclick="return confirm(&quot;{{ tr('admin_user_verify_confirmation', $user_details->name) }}&quot;)">

         <span class="text-danger">Verify</span>
